try {
  ...
  (function(d, q) {
    var r = function(a, b) {
      ...
      e = r("childList attributes characterData subtree attributeOldValue characterDataOldValue attributeFilter".split(
        " "), function(a) {
    ...
    k.prototype._matchRemovedNodes = function(a) {
      ...
      a.previousSibling ? b.find(a.previousSibling).after(c) : a.nextSibling ? b.find(a.nextSibling).before(c) : (
        this.target === a.target ? b.root : b.find(a.target)).empty().append(c);
    ...
    k.prototype._matchAttributeFilter = function(a) {
      return this.attributeFilter && this.attributeFilter.length ? 0 <= this.attributeFilter.indexOf(a.attributeName) :
        !0
    };
    var n = function(a) {
      this.patterns = [];
    ...
    n.prototype._collapseOptions = function() {
      ...
      this.patterns.forEach(function(b) {
        ...
        if (!c && a.attributes || !b.attributeFilter) c && b.options.attributes && !b.attributeFilter && delete a
          .attributeFilter;
        else {
          var e = {},
            f = [];
  ...
  $('div.error-box')
    .observe({
      ...
      if (record.target.className != "col-xs-12 error-box hidden") //desactivamos cuando se ocultan los mensajes

      // console.log(record.target.children[0].children);
        var error_el = record.target.children[0].children;
      if (error_el) {
        for (index = 0; index < error_el.length; ++index) {
          console.log(error_el[index].innerText);
          dataLayer.push({
            'event': 'eventga',
            'category': 'Form Error',
            'action': '{{Page Path}}',
            'label': error_el[index].innerText || undefined,
            'value': undefined,
            'nonInteraction': true
          });
        }
